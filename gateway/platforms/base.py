...
def merge_pending_message_event(
    ...
    *,
    ...
    if existing:
        # Waitable internal callers (notably Discord Realtime voice) attach
        # completion futures to their synthetic event. Preserve those futures
        # across coalescing/replacement so no caller waits forever.
        existing_waiters = list(getattr(existing, "_completion_futures", []) or [])
        incoming_waiters = list(getattr(event, "_completion_futures", []) or [])
        if incoming_waiters:
            existing._completion_futures = existing_waiters + incoming_waiters
        if existing_waiters:
            event._completion_futures = existing_waiters + incoming_waiters
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
            ...
            if should_bypass_active_session(cmd):
                ...
                if cmd and is_interrupt_then_dispatch(cmd):
                    ...
                    try:
                        ...
                        self._resolve_message_completion(
                            event, f"Hermes command /{cmd} was handled."
                        )
                    except Exception as e:
                        self._resolve_message_completion(
                            event, f"Hermes command /{cmd} failed: {e}"
                        )
                        logger.error(
                ...
                try:
                    ...
                    self._resolve_message_completion(event, _text)
                return
            ...
            if not cmd and event.allow_gateway_control:
                ...
                if _has_text_clarify:
                    ...
                    try:
                        ...
                        self._resolve_message_completion(
                            event, f"Hermes clarification failed: {e}"
                        )
                        logger.error(
            ...
            if self._busy_session_handler is not None:
                try:
                    if await self._busy_session_handler(event, session_key):
                        inline_response = getattr(
                            event, "_inline_completion_response", None
                        )
                        if inline_response is not None:
                            self._resolve_message_completion(event, inline_response)
                        return
                except Exception as e:
        ...
        if not started:
            self._resolve_message_completion(
                event, "Hermes could not start this message turn."
            )

    async def handle_message_and_wait(self, event: MessageEvent) -> str:
        """Process through the normal queue and await the handler's final text.

        Platform ingress normally returns immediately. Internal transports that
        must relay Hermes' answer elsewhere can use this without bypassing the
        adapter's session serialization, interruption, or persistence rules.
        """
        loop = asyncio.get_running_loop()
        future: asyncio.Future = loop.create_future()
        waiters = list(getattr(event, "_completion_futures", []) or [])
        waiters.append(future)
        event._completion_futures = waiters
        if not self._message_handler:
            future.set_result("")
        else:
            await self.handle_message(event)
        return str(await asyncio.shield(future) or "")

    @staticmethod
    def _resolve_message_completion(event: MessageEvent, response: Any) -> None:
        """Resolve any private waiters attached by ``handle_message_and_wait``."""
        waiters = list(getattr(event, "_completion_futures", []) or [])
        if not waiters:
            return
        event._completion_futures = []
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if completion_response is None:
                completion_response = getattr(event, "_streamed_final_response", None)
            if not response:
            ...
            completion_response = "Hermes stopped before replying."
            ...
            completion_response = (
                f"Hermes hit an error: {type(e).__name__}: {str(e)[:300]}"
            )
            ...
            self._resolve_message_completion(event, completion_response)
