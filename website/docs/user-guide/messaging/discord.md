...
- **Connect**, **Speak**, and **Use Voice Activity** — required if Hermes will
  join and talk in voice channels (including Realtime voice)
...
| Recommended + voice | `274914987072` | Recommended plus Connect, Speak, and Use Voice Activity |
...
# Optional: required only for low-latency Realtime voice channels
# OPENAI_API_KEY=sk-...
```
...
### Realtime voice channels

Hermes can use the native Discord mobile app as a low-latency, interruptible
voice client. No webpage or public microphone endpoint is involved:

```text
Discord mobile VC ↔ Hermes Discord bot ↔ OpenAI Realtime ↔ Hermes agent
```

Every spoken turn is forced through the normal Hermes session, preserving its
tools, memory, text history, and authorization. OpenAI Realtime provides turn
detection and speech only. If Realtime cannot start or disconnects, the voice
receiver falls back to Hermes' existing utterance transcription + TTS path.

Set the secret in `~/.hermes/.env`:

```bash
OPENAI_API_KEY=sk-...
```

Enable the behavior in `~/.hermes/config.yaml`:
...
Restart the gateway, join a Discord voice channel, then run `/voice join`.
