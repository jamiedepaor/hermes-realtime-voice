...
class TestVoiceChannelCommands:
    ...
    @pytest.mark.asyncio
    async def test_realtime_input_waits_for_hermes_without_legacy_tts(self, runner):
        """Realtime uses the same session pipeline but owns audio playback."""
        from gateway.config import Platform
        ...
        mock_adapter._client = MagicMock()
        mock_adapter._client.get_channel = MagicMock(return_value=AsyncMock())
        mock_adapter.handle_message_and_wait = AsyncMock(return_value="Hermes reply")
        runner.adapters[Platform.DISCORD] = mock_adapter
        ...
        assert result == "Hermes reply"
        event = mock_adapter.handle_message_and_wait.call_args[0][0]
