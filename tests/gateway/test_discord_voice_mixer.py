...
class TestVoiceMixerCore:
    ...
    def test_streaming_speech_preserves_partial_frame_order(self):
        mx = vm.VoiceMixer()
        pcm = np.arange(vm.FRAME_SIZE // 2, dtype=np.int16).tobytes()
        mx.start_speech_stream("reply", fade_in_ms=0)
        mx.append_speech_stream("reply", pcm[:1000])
        assert mx.read() == vm.SILENCE_FRAME

        mx.append_speech_stream("reply", pcm[1000:])
        assert mx.read() == pcm
        mx.finish_speech_stream("reply")
        assert mx.read() == vm.SILENCE_FRAME
        assert mx.speech_active is False

    def test_cancel_stream_reports_consumed_audio_ms(self):
        mx = vm.VoiceMixer()
        mx.start_speech_stream("reply", fade_in_ms=0)
        mx.append_speech_stream("reply", b"\x01\x00" * (vm.FRAME_SIZE // 2))
        mx.read()
        assert mx.cancel_speech_stream("reply") == vm.FRAME_LENGTH_MS
        assert mx.speech_active is False
