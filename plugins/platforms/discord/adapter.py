...
class VoiceReceiver:
    ...
    def __init__(
        ...
        # Optional low-latency path. Called on discord.py's socket-reader
        # thread after Opus decode. Returning True means the chunk was consumed
        # and must not also enter the legacy silence/Whisper buffer.
        ...
        try:
            ...
            if user_id and self._pcm_callback is not None:
                try:
                    ...
                    logger.debug(
                        "Realtime PCM callback failed for user %s: %s",
                        user_id, callback_error,
                    )
            if consumed:
                return
            with self._lock:
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._realtime_voice_agent_callback: Optional[Callable] = None  # set by run.py
        self._realtime_voice_sessions: Dict[int, Any] = {}
        self._realtime_voice_cfg: Dict[str, Any] = self._load_realtime_voice_config()
        self._voice_event_loop: Optional[asyncio.AbstractEventLoop] = None
    ...
    def _load_realtime_voice_config(self) -> Dict[str, Any]:
        """Read non-secret ``discord.realtime_voice`` settings.

        The API key intentionally does not belong here; it is resolved from
        the active profile's secret scope only when a voice session starts.
        """
        defaults: Dict[str, Any] = {
            "enabled": False,
        ...
        try:
            from hermes_cli.config import read_raw_config
            cfg = read_raw_config() or {}
            realtime = ((cfg.get("discord") or {}).get("realtime_voice") or {})
            if isinstance(realtime, dict):
                for key, value in realtime.items():
                    if key in defaults and value is not None:
                        defaults[key] = value
        except Exception as exc:
            logger.debug("Could not load discord.realtime_voice config: %s", exc)
        return defaults
    ...
    def realtime_voice_enabled(self) -> bool:
        raw = (getattr(self, "_realtime_voice_cfg", None) or {}).get("enabled", False)
        return raw is True or str(raw).strip().lower() in {"1", "true", "yes", "on"}

    def realtime_voice_active(self, guild_id: int) -> bool:
        session = (getattr(self, "_realtime_voice_sessions", None) or {}).get(guild_id)
        return bool(session and session.is_connected)

    async def _start_realtime_voice_session(self, guild_id: int) -> bool:
        """Start the Realtime provider for a connected guild, if configured."""
        if not self.realtime_voice_enabled() or self._realtime_voice_agent_callback is None:
            return False
        mixer = self._voice_mixers.get(guild_id)
        if mixer is None:
            return False
        ...
        try:
            ...
            if not api_key:
                # Compatibility with Hermes' existing OpenAI voice-tool key.
                api_key = (get_secret("VOICE_TOOLS_OPENAI_KEY", "") or "").strip()
        except Exception as exc:
            logger.warning(
                "Discord Realtime voice could not resolve its profile-scoped API key: %s",
                exc,
            )
            return False
        if not api_key:
            logger.warning(
                "Discord Realtime voice is enabled but OPENAI_API_KEY is not set; "
                "using standard Whisper/TTS voice mode"
            )
            return False

        try:
            from .realtime_voice import RealtimeVoiceSession

            cfg = self._realtime_voice_cfg

            async def _consult(gid: int, uid: int, request: str) -> str:
                callback = self._realtime_voice_agent_callback
                if callback is None:
                    raise RuntimeError("Hermes Realtime callback is not connected")
                return await callback(guild_id=gid, user_id=uid, transcript=request)
            ...
            session = RealtimeVoiceSession(
                ...
                max_tool_output_chars=int(cfg.get("max_tool_output_chars") or 12000),
            )
            await session.start()
            ...
            logger.warning(
                "Discord Realtime voice failed to start (guild=%d): %s; "
                "using standard Whisper/TTS voice mode",
                guild_id, exc,
            )
            return False

    def _feed_realtime_voice_audio(self, guild_id: int, user_id: int, pcm: bytes) -> bool:
        """Socket-thread bridge into a guild's Realtime audio queue."""
        ...
        if not self._is_allowed_user(str(user_id), guild=guild, is_dm=False):
            return True  # drop unauthorized audio without paying to transcribe it
        consumed = session.feed_audio_threadsafe(user_id, pcm)
        loop = self._voice_event_loop
        if consumed and loop is not None:
    ...
    async def join_voice_channel(self, channel, *, text_channel_id: int = None, source: dict = None) -> bool:
        ...
        async with self._voice_locks.setdefault(guild_id, asyncio.Lock()):
            ...
            # Realtime also needs the mixer, even when ambient voice FX are off.
            if getattr(self, "_voice_fx_cfg", {}).get("enabled") or self.realtime_voice_enabled():
                try:
                    await self._install_voice_mixer(guild_id, vc)
                except Exception as e:
                    logger.warning("Voice mixer failed to start: %s", e)

            realtime_started = await self._start_realtime_voice_session(guild_id)

            # Start voice receiver (Phase 2: listen to users). Realtime consumes
            # decoded frames immediately; if it is unavailable or disconnects,
            # chunks fall through to the existing silence/Whisper path.
            try:
                pcm_callback = None
                if realtime_started:
                    pcm_callback = lambda uid, pcm: self._feed_realtime_voice_audio(
                        guild_id, uid, pcm
                    )
